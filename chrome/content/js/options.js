...
Lazarus.Options = {
  ...
  initSyncTab: function(){
    ...
    $('#sync-stop').click(function(){
      if (confirm(Lazarus.locale.getString('options.sync.clearServerData.confirm'))){
        Lazarus.Options.clearServerData(function(response){
          if (response.errorMessages){
            Lazarus.msg(response.errorMessages, "error");
          }
          else {
            Lazarus.msg("options.sync.clearServerData.success", "success");
          }
          //either way, stop syncing this browser
    ...
    $('#sync-login').click(function(){
      ...
      if (!email){
        ...
        var passwordHash = Lazarus.Sync.secureHash(password);
    ...
    $('#sync-reset-password-link').click(function(){
      ...
      var url = Lazarus.SYNC_SERVER +'/user/resetPassword';
    ...
    $('#sync-create-account').click(function(){
      ...
      if (!email){
        ...
        //all attempt to create the account on the server
        var syncKey = Lazarus.Sync.generateSyncKey(email, password);
        var passwordHash = Lazarus.Sync.secureHash(password);
        var syncKeyHash = Lazarus.Sync.secureHash(syncKey);
        
        //send the passwordHash (for user login verification) 
        //syncKeyHash (syncKey verification)
        ...
        Lazarus.callBackground('Lazarus.Sync.callAPI', ['/user/create', {
          ...
          if (response.errorMessages){
            ...
            Lazarus.callBackground('Lazarus.setPref', ["syncKey", syncKey, function(){
              Lazarus.callBackground('Lazarus.setPref', ["syncKeyHash", syncKeyHash, function(){
    ...
    $('#sync-reset-database').click(function(){
      if (confirm(Lazarus.locale.getString('options.sync.resetWarning'))){
        Lazarus.getPref('userId', function(userId){
          Lazarus.callBackground('Lazarus.Sync.callAPI', ['/user/resetDatabase', {}, function(response){
            if (response.errorMessages){
              ...
              //database reset, resync the database
